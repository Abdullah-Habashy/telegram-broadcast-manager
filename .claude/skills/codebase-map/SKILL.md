...
ops-health-check.sh    مراقبة كل ٥ دقايق + تنبيه تيليجرام — /etc/cron.d كمان
```
...
| مراجعة الفيديوهات | `video_books` (مندليف/مذكرة/شرح)، `review_videos` (+`chapter` ١-٥ و`video_number` ١-١٥)، `video_review_notes` (+`severity`) |
...
**الموظف بيشوف الإجمالي دلوقتي (٧ سبتمبر ٢٠٢٦).** كان متشال عن قصد — الأرقام مكانتش بتتبعت
في الرد أصلًا مش متخبّية في الواجهة — عشان مايقدرش يقدّر عدد الطلاب على المنصة. **القرار
اتعكس بطلب صاحب المشروع:** الموظف محتاج يعرف حجم شغله وقد إيه الفلتر ضيّق النتيجة، وده
مستحيل من غير رقم. `total` و`pages` بيتبعتوا للكل، و"محتاجة تدخّلك: N" (`actionable`) فضلت
للموظف **جنبه مش بدله** — الإجمالي بيقول «عندك كام» وهي بتقول «ابدأ بأنهي».
**متشيلهوش تاني من غير سؤال.**

الرقم بيتحسب من نفس `where` بتاع الفلاتر، فبيتحدّث مع أي فلتر. والواجهة بتتفرّع على **وجود**
`data.total` مش على الدور، فلو القاعدة اتغيّرت على السيرفر الشاشة بتمشي وراها من غير تعديل.
...
`HH:MM:SS` — **وكل الترقيم قوايم مقفولة مش كتابة** (الباب ١-٥، الدرس ١-١٥، الساعة والدقيقة
والثانية)، والكتابة الوحيدة اسم الدرس والتعليق. حدود الترقيم مكرّرة في `schema.sql` والكنترولر
والقالب — التلاتة مع بعض. كل غلطة ليها `severity` (لازم/الأفضل/سهلة، الافتراضي `must`).
التعليق بيتعرض `dir="rtl"` مش `auto` (المصطلح الإنجليزي في أول الجملة كان بيقلبها كلها). التصدير PDF بـpuppeteer وبيضمّن الصور base64 للمسارات المحلية.
...
**المراقبة شغّالة** (١٩ سبتمبر ٢٠٢٦): `/root/health-check.sh` من
`/etc/cron.d/telegram-health-check` كل ٥ دقايق — بيفحص الخدمة، ورد التطبيق فعليًا على
`127.0.0.1:3000`، والتانل، والموقع من بره، و`postgres`، والقرص؛ وبيبعت على تيليجرام لحساب
الأدمن (`settings.forward_chat_id`). **التنبيه عند تغيّر الحالة بس** — وقوع، رجوع، وتذكير
كل ٦ ساعات. المصدر `ops-health-check.sh` في المستودع، والتوكن مفكوك في
`/root/health-alert.env` (600) عشان التنبيه يشتغل والقاعدة واقعة، وبيتحدّث من القاعدة
لوحده. ⚠️ **بيموت مع السيرفر** — لو الجهاز كله وقع مفيش حد هيبعت؛ لده `PING_URL`
(dead-man switch خارجي) في نفس الملف، لسه مش مضبوط.
