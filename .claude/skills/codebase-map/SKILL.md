...
**انشر مباشرة بدون استئذان — موافقة دائمة من صاحب المشروع.** بعد التعديل والتحقق والكوميت، كمّل للنشر على طول وبلّغ بالنتيجة بعد الفعل. الاستثناء الوحيد اللي بيتعرض قبل التنفيذ: تعديل schema **مدمّر أو غير قابل للرجوع** (`DROP TABLE` / `DROP COLUMN`، أو `UPDATE`/`DELETE` بيضيّع بيانات إنتاج فعلية) — إضافة أعمدة وفهارس بـ `IF NOT EXISTS` نشر عادي.
